...
class ChefProvider {
  ...
  // TODO: Implement dynamic-scope + expansion of options
	...
	inflateSuggestion(suggestion) {
		...
		// TODO: Change to default image, only if no other is given -> in a kind of: icon.nil? -> go on with defaults
		let cicon;
		if ( suggestion.type == 'Resource' ) {
			cicon = '<div style="background-color:#b6dddc;"><i class="icon-heart"></i></div>';
		} else if ( suggestion.type == 'Helper' ){
			cicon = '<div style="color:Red; background-color:#b6dddc;"><i class="icon-heart"></i></div>';
		} else if ( suggestion.type == 'Metadata' ) {
			cicon = '<div style="color:Brown; background-color:#deb6d0;"><i class="icon-squirrel"></i></div>';
		} else {
			cicon = '<div style="color:Brown; background-color:#b9deb6;"><i class="icon-squirrel"></i></div>';
		}
